...
@section('title', 'Edit Product - Maison de Hoor')
                ...
                <div class="card-body">
                    <form action="{{ route('products.update', $product) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            ...
                            @if ($product->image)
                                <div class="mb-2">
                                    <img src="{{ route('products.media', $product) }}" alt="{{ $product->name }}" style="max-width: 200px;">
                                </div>
                            @endif
                            <input type="file" id="image" name="image" class="form-control @error('image') is-invalid @enderror" 
                                   accept="image/*">
                            <small class="text-muted">Max 2MB. Formats: JPG, PNG, GIF. Leave empty to keep current image.</small>
                            @error('image')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
